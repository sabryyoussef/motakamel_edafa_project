...
class SchoolAnnouncement(models.Model):
    ...
    name = fields.Char(string='Code No:', default=lambda x: _('New'),
                       help="Code for the announcement.")
    announcement_subject = fields.Char(string='Subject', required=True,
                                       help="Title or Subject of the Announcement")
    state = fields.Selection([('draft', 'Draft'), ('to_send', 'Confirmed'),
                              ('pending', 'Pending'), ('done', 'Done'),
                              ('failed', 'Failed'), ('expired', 'Expired')],
                             string='Status', default='draft', copy=False,
                             help="State of the announcement")
    scheduled_date = fields.Date(string='Scheduled Date',
                                 help="If set, the queue manager will send the "
                                      "email after the date. If not set, the "
                                      "email will be send as soon as possible.")
    requested_date = fields.Date(string='Send Date',
    ...
    to_assign = fields.Selection([('general', 'General'),
                                  ('student', 'Student'),
                                  ('staff', 'Staff')],
                                 string='Recipients',
                                 required=True,
    ...
    def _convert_inline_images_to_urls(self, body_html):
        """
        Find inline base64 encoded images, make an attachment out of
        them and replace the inline image with an url to the attachment.
        """
